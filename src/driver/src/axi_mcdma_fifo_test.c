...
int axi_mcdma_fifo_test(int payload_len) {
	...
	// axi mcdma destination addr
	uint32_t * axis_buf_desc_mm2s_v_addr = (uint32_t *) mmap(NULL, AXI_MCDMA_BUF_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, dev_fd, AXI_MCDMA_MM2S_BD_CHAIN_ADDR); 
	...
	uint32_t * axis_buf_desc_s2mm_v_addr = (uint32_t *) mmap(NULL, AXI_MCDMA_BUF_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, dev_fd, AXI_MCDMA_S2MM_BD_CHAIN_ADDR); 
	...
	_reg_set(axis_buf_desc_mm2s_v_addr, AXI_MCDMA_MM2S_BD_NEXT_DESC_LSB, AXI_MCDMA_MM2S_BD_CHAIN_ADDR);
	_reg_set(axis_buf_desc_mm2s_v_addr, AXI_MCDMA_MM2S_BD_BUF_ADDR_LSB, AXI_MCDMA_BUF_SRC_ADDR);
	_reg_set(axis_buf_desc_mm2s_v_addr, AXI_MCDMA_MM2S_BD_STATUS, 0x00000000);
	_reg_set(axis_buf_desc_mm2s_v_addr, AXI_MCDMA_MM2S_BD_CONTROL, (uint32_t) (payload_len * sizeof(uint32_t)) | 0xc0000000);
	fprintf(stderr, "mm2s Buffer Length: 0x%08x\n", (uint32_t) (payload_len*2 * sizeof(uint16_t)) | 0xc0000000);

	_reg_set(axis_buf_desc_s2mm_v_addr, AXI_MCDMA_MM2S_BD_NEXT_DESC_LSB, AXI_MCDMA_S2MM_BD_CHAIN_ADDR);
	...
	_reg_set(axi_mcdma_v_addr, AXI_MCDMA_MM2S_CH1CURDESC_LSB, AXI_MCDMA_MM2S_BD_CHAIN_ADDR);
	fprintf(stderr, "mm2s: Set channel 0 current bd to address: 0x%08x\n", AXI_MCDMA_MM2S_BD_CHAIN_ADDR);
	...
	uint32_t td_value_mm2s = AXI_MCDMA_MM2S_BD_CHAIN_ADDR;
	// uint32_t td_value_mm2s = AXI_MCDMA_MM2S_BD_CHAIN_ADDR;
	...
	_reg_set(axi_mcdma_v_addr, AXI_MCDMA_S2MM_CH1CURDESC_LSB, AXI_MCDMA_S2MM_BD_CHAIN_ADDR);
	fprintf(stderr, "s2mm: Set channel 0 bd to dst address: 00x%08x\n", AXI_MCDMA_S2MM_BD_CHAIN_ADDR);
	...
	uint32_t td_value_s2mm = AXI_MCDMA_S2MM_BD_CHAIN_ADDR << 6;
	...
	while (!(s2mm_sr & AXI_MCDMA_MM2S_CHIDLE) || !(s2mm_sr & AXI_MCDMA_MM2S_HALTED)) {
	// while (!(mm2s_sr)) {
